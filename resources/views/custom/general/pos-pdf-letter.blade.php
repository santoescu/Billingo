...
<head>
    ...
    <style>
        ...
        .doc-title { font-size: 20px; font-weight: bold; margin: 0; color: #1f2933; }
        ...
        .doc-uuid { font-size: 8px; word-break: break-all; color: #9ca3af; margin: 2px 0 0; }
    </style>
</head>
<body>
    <table class="header-table">
        <tr>
            ...
            <td class="end" style="width: 40%;">
                <p class="doc-title">{{ $isElectronic ? __('SALES INVOICE') : __('REMISION') }}</p>
                <p class="doc-numeral">{{ $documento->numeral }}</p>
                <p class="doc-numeral">{{ optional($documento->issue_date)->format('Y-m-d H:i') }}</p>
                @unless ($isElectronic)
                    <p class="doc-numeral bold">{{ __('Not an electronic invoice') }}</p>
                @endunless
            </td>
        </tr>
    </table>
    ...
    <table class="totals">
        ...
        @if ($cashReceived !== null)
            ...
            @endphp
            <tr>
                <td class="muted">{{ __('Cash received') }}</td>
    ...
    @if ($isElectronic)
        <p class="doc-uuid" style="margin-top: 26px;">CUFE: {{ $uuid }}</p>
    @else
        <p class="footer-note">{{ __('REMISION - not an electronic invoice') }}</p>
    @endif
</body>
</html>
