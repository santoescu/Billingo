...
<body>
    ...
    @if ($isElectronic)
        <p class="doc-uuid center">{{ $uuid }}</p>
    @endif
    ...
    <table class="totals">
        ...
        @if ($cashReceived !== null)
            ...
            <tr>
                <td class="muted">{{ __('Change') }}</td>
                <td class="end">{{ number_format(max((float) $cashReceived - $cashPortion, 0), 2) }}</td>
            </tr>
        @endif
        <tr class="total-row">
            <td>{{ __('Total') }}</td>
    ...
    <p class="footer-note">{{ __('Thank you for your purchase!') }}</p>
</body>
</html>
