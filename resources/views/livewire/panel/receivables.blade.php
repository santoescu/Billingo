    ...
    <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-800">
        ...
        <dl class="space-y-2 mb-4">
            <div class="flex justify-between items-baseline">
                ...
                <dd class="text-sm font-medium text-gray-800 dark:text-white">{{ $receivables['pending_count'] }} &middot; $<x-panel.count-up :target="$receivables['total_pending']" :decimals="2" /></dd>
            </div>
            <div class="flex justify-between items-baseline">
                <dt class="text-xs text-zinc-500 dark:text-neutral-400">{{ __('Overdue') }}</dt>
                <dd class="text-sm font-medium {{ $receivables['overdue_count'] > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-white' }}">{{ $receivables['overdue_count'] }} &middot; $<x-panel.count-up :target="$receivables['total_overdue']" :decimals="2" /></dd>
            </div>
        </dl>
