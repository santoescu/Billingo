<x-layouts.app :title="__('Quotations')">
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="border border-gray-200 rounded-lg divide-y divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
            ...
            <table id="quotationsTable" class="w-full table-fixed divide-y divide-gray-200 dark:divide-neutral-700">
    ...
    @push('scripts')
        <script>
            // Instancia viva de la tabla -- se crea una sola vez en loadQuotationsTable() y de
            // ahí en adelante cada refresh solo le reemplaza las filas (clear/rows.add) en vez
            // de destruirla y reconstruirla (ver documents/index.blade.php para el porqué:
            // destroy() restaura el <tbody> al contenido del primer init).
            let quotationsTable = null;

            function escapeHtmlQuotationRow(value) {
                return String(value ?? '').replace(/[&<>"']/g, (char) => ({
                    '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;',
                })[char]);
            }

            function renderQuotationCustomer(row) {
                const dv = row.customer_dv ? '-' + escapeHtmlQuotationRow(row.customer_dv) : '';
                return `<div class="text-gray-800 dark:text-neutral-200">${escapeHtmlQuotationRow(row.customer_name ?? '—')}</div>`
                    + `<div>${escapeHtmlQuotationRow(row.customer_identification ?? '—')}${dv}</div>`;
            }

            function renderQuotationStatus(row) {
                return `<span class="rounded-md px-2 py-0.5 text-xs font-medium ${row.status_badge_classes}">${escapeHtmlQuotationRow(row.status_label)}</span>`;
            }

            function renderQuotationActions(row) {
                return `<div class="flex justify-end gap-1">
                    <a href="${row.urls.preview}" target="_blank" class="flex size-8 items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 hover:text-accent focus:outline-hidden dark:text-neutral-400 dark:hover:bg-neutral-700" aria-label="{{ __('View PDF') }}" title="{{ __('View PDF') }}">
                        <svg class="size-4 shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/></svg>
                    </a>
                    <a href="${row.urls.show}" class="quotation-view-btn flex size-8 items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 hover:text-accent focus:outline-hidden dark:text-neutral-400 dark:hover:bg-neutral-700" aria-label="{{ __('View') }}" title="{{ __('View') }}">
             ...
             * loadDocumentsTable() en documents/index.blade.php. El backend
             * manda el JSON crudo (data.rows) y las funciones render*() de
             * arriba arman cada celda.
             * @returns {void}
             */
            function loadQuotationsTable() {
                ...
                fetch('{{ route('quotations.data') }}', { headers: { Accept: 'application/json' } })
                    ...
                    .then((data) => {
                        if (! quotationsTable) {
                            quotationsTable = initWorkflowDataTable('#quotationsTable', '#hs-table-with-pagination-search', {
                                emptyTable: "{{ __('There are no registered :name.', ['name' => __('Quotations')]) }}",
                                columns: [
                                    { data: 'issue_date', defaultContent: '—', className: 'px-4 py-4 text-sm text-gray-600 dark:text-neutral-400' },
                                    { data: 'numeral', className: 'px-4 py-4 text-sm font-medium text-gray-800 break-words dark:text-neutral-200' },
                                    { data: null, className: 'px-4 py-4 text-sm text-gray-600 dark:text-neutral-400', render: (data, type, row) => renderQuotationCustomer(row) },
                                    { data: 'total_formatted', className: 'px-4 py-4 text-sm text-gray-600 dark:text-neutral-400' },
                                    { data: null, className: 'px-4 py-4 text-sm', render: (data, type, row) => renderQuotationStatus(row) },
                                    { data: null, orderable: false, className: 'px-4 py-4 text-end text-sm', render: (data, type, row) => renderQuotationActions(row) },
                                ],
                            });
                        }

                        quotationsTable.clear();
                        quotationsTable.rows.add(data.rows);
                        quotationsTable.order([]).draw();
                    })
                    .finally(() => {
