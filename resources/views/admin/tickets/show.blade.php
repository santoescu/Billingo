            ...
            <div class="flex min-h-0 flex-1 flex-col gap-6 lg:flex-row">
                ...
                <div class="order-2 w-full shrink-0 space-y-6 lg:order-none lg:w-64">
                    ...
                    @if ($ticket->satisfaction_rating)
                        <div>
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-neutral-500">{{ __('Satisfaction') }}</label>
                            <div class="flex items-center gap-0.5 text-amber-400">
                                @for ($i = 1; $i <= 5; $i++)
                                    <svg class="size-4 shrink-0 {{ $i <= $ticket->satisfaction_rating ? 'fill-current' : 'fill-none text-zinc-300 dark:text-neutral-600' }}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11.525 2.295a.53.53 0 0 1 .95 0l2.31 4.679a2.123 2.123 0 0 0 1.595 1.16l5.166.756a.53.53 0 0 1 .294.904l-3.736 3.638a2.123 2.123 0 0 0-.611 1.878l.882 5.14a.53.53 0 0 1-.771.56l-4.618-2.428a2.122 2.122 0 0 0-1.973 0L6.396 21.01a.53.53 0 0 1-.77-.56l.881-5.139a2.122 2.122 0 0 0-.611-1.879L2.16 9.795a.53.53 0 0 1 .294-.906l5.165-.755a2.122 2.122 0 0 0 1.597-1.16z"/></svg>
                                @endfor
                            </div>
                            @if ($ticket->satisfaction_comment)
                                <p class="mt-1 text-xs text-zinc-500 dark:text-neutral-400">{{ $ticket->satisfaction_comment }}</p>
                            @endif
                        </div>
                    @endif
