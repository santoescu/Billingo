...
<x-layouts.app :title="__('Activity log')">
    @include('partials.tittle', [
        ...
        'subheading' => __('Everything created, edited or deleted across every company.'),
    ])
    ...
    <div class="flex flex-col gap-4">
        <form method="GET" action="{{ route('admin.activity-log.index') }}" class="flex flex-wrap items-end gap-3">
            <div class="w-56">
                ...
                <select name="company_id" class="activity-log-filter-auto-submit hidden" data-hs-select='{!! \App\Support\SelectConfig::searchable(__('All'), __('Search...')) !!}'>
            ...
            <div class="w-48">
                ...
                <select name="user_id" class="activity-log-filter-auto-submit hidden" data-hs-select='{!! \App\Support\SelectConfig::searchable(__('All'), __('Search...')) !!}'>
                    <option value="">{{ __('All') }}</option>
                    @foreach ($users as $filterUser)
                        <option value="{{ $filterUser->_id }}" @selected(($filters['user_id'] ?? '') === (string) $filterUser->_id)>{{ $filterUser->name }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="-m-1.5 overflow-x-auto">
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="border border-gray-200 rounded-lg divide-y divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
                    @include('activity-log.partials.table', ['logs' => $logs, 'userNames' => $userNames, 'companyNames' => $companyNames])
                </div>
            </div>
        </div>
    </div>
