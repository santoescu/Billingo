...
class UblDocumentBuilder
{
    ...
    public function build(Company $company, array $payload): string
    {
        ...
        $lineas = $this->buildLineasCalculadas($payload['lineas'] ?? []);
        ...
        $cargos = $this->buildCargosCalculados($payload['cargos_descuentos'] ?? [], $lineExtensionAmount);
     ...
     */
    private function buildLineasCalculadas(array $lineasPayload): array
    {
     ...
     */
    private function agruparImpuestos(array $lineas): array
    {
        $grupos = [];
     ...
     */
    private function buildCargosCalculados(array $cargosPayload, float $baseAmount): array
    {
     ...
     */
    private function calcularTotales(float $lineExtensionAmount, array $impuestos, array $cargos): array
    {
     ...
     */
    private function nombreImpuesto(string $codigo): string
    {
        return match ($codigo) {
            '01' => 'IVA',
            '03' => 'ICA',
