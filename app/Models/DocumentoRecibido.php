...
class DocumentoRecibido extends Model
{
    ...
    // Mismos códigos que DocumentoEmitido::STATUS_* -- "claimed" es propio de
    // recepción (RADIAN: reclamo de una factura recibida), no existe del lado emisión.
    ...
    public const STATUS_CLAIMED = 4;

    // Mismo código que DocumentoEmitido::PAYMENT_MEANS_CREDIT -- "cac:PaymentMeans/cbc:ID" del
    // XML (1=contado, 2=crédito), no confundir con "PaymentMeansCode" (el medio en sí: efectivo,
    // transferencia, etc.).
    ...
    {
        return $this->belongsTo(ThirdParty::class, 'proveedor_id');
    }
    ...
    public function getStatusLabelAttribute(): string
    {
        return match ($this->status) {
            ...
            self::STATUS_CLAIMED => __('Claimed'),
    ...
    public function getStatusBadgeClassesAttribute(): string
    {
        return match ($this->status) {
            ...
            self::STATUS_CLAIMED => 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300',
